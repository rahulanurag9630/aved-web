...
const PropertyFilterDropdown = () => {
  ...
  const handleSearch = async () => {
    ...
    const { propertyType, bedroom, priceRange, availability } = selectedOptions;

    // Utility to parse number, handling "10+" as MAX_SAFE_INTEGER
    ...
    console.log({
      property_type: propertyType?.[0] || undefined,
      no_of_bedrooms: parseNumber(bedroom?.[0]),
      min_price: parseNumber(priceRange?.min),
      max_price: parseNumber(priceRange?.max),
      status: availability?.[0] === "Available" ? "ACTIVE" : "BLOCK",
      limit: 10,
      page: 1,
    })
    router.push({
