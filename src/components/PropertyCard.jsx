...
export default function PropertyCard({ property }) {
  ...
  const getTimeRemaining = (deadline) => {
    ...
    if (diff <= 0) return `${t("expired")}`; // If deadline has passed
  ...
  return (
    <StyledCard>
      <ImageContainer>
        ...
        />
        <SwiperTopRightChip
          label={`${t("strategy")} : ${(property?.investmentStrategy === "GROWTH" &&
              t("property_brief.growth")) ||
            (property?.investmentStrategy === "MONTHLY_INCOME" &&
              t("property_brief.monthlyIncome")) ||
            (property?.investmentStrategy === "ANNUAL_INCOME" &&
              t("property_brief.annualIncome")) ||
            (property?.investmentStrategy === "SEMI_ANNUAL_INCOME" &&
              t("property_brief.semiAnnualIncome")) ||
            (property?.investmentStrategy === "VALUE" &&
              t("property_brief.value")) ||
            (property?.investmentStrategy === "EXIT" &&
              t("property_brief.exit"))
            }`}
          size="small"
        />
        <SwiperTopLeftChip1
          label={`${t("risk")} : ${(property?.investmentRiskLevel === "LOW" &&
      ...
      <CardContent sx={{ pl: 0, pr: 0 }}>
        <Box sx={{ mb: 0 }}>
          <Box sx={{ display: "flex", gap: 0.4, mb: 1, flexWrap: "wrap" }}>
            <StyledChip
              ...
              label={`${t("from")} ${formatDate(
                property?.dateIssuedOnPlatform
              )}`}
              variant="outlined"
              size="small"
              sx={{
        ...
        <Box sx={propDescStyle(property)}>
          ...
          {user?.userInfo?.verifyAccount === false ? (
            ...
            <>
              {" "}
              {property?.projectGrossYield_appearance && <StatRow sx={style.borderBottom}>
              ...
              {property?.projectNetYield_appearance && <StatRow sx={style.borderBottom}>
                <Typography variant="body4" color="secondary">
                  {t("net_rental_yield")}
                </Typography>
                <Typography
